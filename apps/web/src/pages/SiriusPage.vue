...
const filteredHistoryRows = computed(() => {
  ...
  return historyRows.value.filter(row =>
    [blueprintName(row.blueprint), row.blueprint.canonicalName, row.blueprint.systemName, row.lastPlanet?.name]
      .filter(Boolean)
      .join(' ')
      .toLowerCase()
      .includes(query),
...
const loadAppearances = async (preferredAppearanceId?: string) => {
  ...
  try {
    ...
    selectedAppearanceId.value =
      preferredId ??
      response.appearances.find(appearance => appearance.id === selectedAppearanceId.value)?.id ??
      response.appearances[0]?.id ??
      ''
  } finally {
    loading.value = false
  }
      ...
      <section class="panel">
        ...
        <form class="form-grid" @submit.prevent="createAppearance">
          ...
          <button
            ...
            :disabled="!appearanceForm.expiresAt || (appearanceForm.planetId === '__new' && appearanceForm.planetName.trim().length < 2)"
          >
      ...
      <section class="panel">
        ...
        <label class="sirius-run-picker">
          ...
          <select id="sirius-selected-appearance-id" v-model="selectedAppearanceId" name="siriusSelectedAppearanceId">
            <option v-for="appearance in appearances" :key="appearance.id" :value="appearance.id">
              {{ appearance.planet.name }} - {{ dateTime(appearance.expiresAt) }}
            </option>
          </select>
        </label>
        ...
        <div v-if="selectedAppearance" class="slot-editor">
          ...
          <div
            ...
            :class="{ 'slot-editor-row-with-enemy': row.slotGroup === 'SLOT_2' }"
          >
            <label>
              {{ t('sirius.slot') }}
              <select
                v-if="selectedRing === 5"
                :id="`sirius-slot-${index}-group`"
                :name="`siriusSlot${index}Group`"
                v-model="row.slotGroup"
                @change="onSlotGroupChanged(row)"
              >
                <option v-for="option in slotOptions" :key="option" :value="option">{{ enumLabel('slot', option) }}</option>
              </select>
              <select
                v-else
                :id="`sirius-slot-${index}-parts`"
                v-model.number="row.partsRequired"
                :name="`siriusSlot${index}Parts`"
                @change="onResourcePartsChanged(row)"
              >
    ...
    <section v-if="canViewSelectedClanDetails" class="panel journey-roadmap-panel">
      ...
      <form v-if="canEditSelectedClan && journeyFormOpen" id="sirius-journey-form" class="journey-form" @submit.prevent="saveJourneyStop">
        ...
        <div class="journey-form-actions">
          <button class="primary-button" :disabled="journeyBusy">
            <Save :size="16" /> {{ isEditingJourneyStop ? t('app.actions.save') : t('sirius.addJourneyStop') }}
          </button>
                ...
                <details v-if="canEditSelectedClan" class="journey-action-menu" @toggle="handleJourneyActionMenuToggle">
                  ...
                  <div class="journey-action-popover" @click="closeJourneyActionMenus()">
                    ...
                    <button class="menu-action" type="button" @click="editJourneyStop(stop)">
                      <Pencil :size="16" /> {{ t('app.actions.edit') }}
                    </button>
                    <button class="menu-action danger-action" type="button" @click="deleteJourneyStop(stop)">
                      <Trash2 :size="16" /> {{ t('app.actions.delete') }}
                    </button>
                  </div>
                </details>
              </div>
            ...
            <tr v-for="row in visibleHistoryRows" :key="row.blueprint.id">
              ...
              <td>
                <span v-if="row.lastPlanet">
                  {{ row.lastPlanet.name }} - {{ t('sirius.ringLabel', { ring: row.lastPlanet.ring ?? '?' }) }}
                </span>
                <span v-else>-</span>
              </td>
