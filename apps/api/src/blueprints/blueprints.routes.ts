...
import { z } from 'zod'
import { bulkUpdateBlueprintStatusSchema, updateBlueprintStatusSchema } from '@bp-tracker/contracts'
...
import { blueprintSummaryInclude, serializeBlueprintSummary } from './blueprint.dto.js'
import { scheduleClanDiscordStatusPublish } from '../notifications/discord-status.service.js'

export const blueprintsRouter = Router()

const normalizeBlueprintStatus = (status: string | null | undefined) => (status === 'OWNED' || status === 'WANTED' ? status : 'MISSING')
...
const createBlueprintSchema = z.object({
  ...
  slotGroup: z.enum(['SLOT_18', 'SLOT_14', 'SLOT_12', 'SLOT_5', 'SLOT_2', 'RESOURCE', 'CUSTOM']).optional(),
  siriusRing: z.number().int().min(1).max(5).nullable().optional(),
  siriusTechTier: z.enum(['OOLYTE', 'DOLOMYTE', 'CLAY', 'KENYTE', 'ANCIENT']).nullable().optional(),
  rarity: z.enum(['STANDARD', 'RARE', 'ANCIENT', 'EVENT', 'CONQUEST', 'SPECIAL', 'COSMETIC']).default('STANDARD'),
...
blueprintsRouter.get(
  ...
  asyncHandler(async (_req, res) => {
    const systems = await prisma.gameSystem.findMany({ orderBy: { sortOrder: 'asc' } })
    res.json({ systems })
  })
)
...
blueprintsRouter.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    const existingBlueprintIds = new Set(existingBlueprints.map(blueprint => blueprint.id))
    const missingBlueprintIds = uniqueBlueprintIds.filter(blueprintId => !existingBlueprintIds.has(blueprintId))

    if (missingBlueprintIds.length > 0) {
      throw new HttpError(404, `Blueprints not found: ${missingBlueprintIds.join(', ')}`)
    }

    const operations = uniqueBlueprintIds.map(blueprintId =>
      prisma.userBlueprintStatus.upsert({
        where: { userId_blueprintId: { userId: req.auth!.user.id, blueprintId } },
        update: { status: input.status, source: 'MANUAL', updatedById: req.auth!.user.id },
        create: {
...
blueprintsRouter.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    const status = await prisma.userBlueprintStatus.upsert({
      where: { userId_blueprintId: { userId: req.auth!.user.id, blueprintId: blueprint.id } },
      update: { status: input.status, source: 'MANUAL', updatedById: req.auth!.user.id },
      create: {
        userId: req.auth!.user.id,
  ...
  asyncHandler(async (req, res) => {
    ...
    const blueprint = await prisma.blueprint.create({
      data: {
        ...
        translations: {
          create: [
            { locale: 'de', name: resolvedNameDe, source: 'admin-create', verified: true },
            ...(resolvedNameEn ? [{ locale: 'en', name: resolvedNameEn, source: 'admin-create', verified: true }] : []),
          ],
        },
      },
      include: blueprintSummaryInclude,
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json({
      members: members.map(member => ({
        ...
        role: member.role,
        status: normalizeBlueprintStatus(member.user.blueprintStatuses[0]?.status),
      })),
    })
  })
)
