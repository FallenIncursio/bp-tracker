...
import { clearSessionCookie, createSession, hashSessionToken, setSessionCookie } from './session.js'
...
const createDiscordAuthorizeUrlForRequest = async (mode: DiscordAuthMode, redirect: string, clanId?: string) => {
  ...
  if (clanId) {
    ...
    const clan = await prisma.clan.findUnique({ where: { id: parsedClanId.data } })
...
const ensurePendingClanMembership = async (userId: string, clanId: string | undefined) => {
  if (!clanId) return

  const existing = await prisma.clanMembership.findUnique({ where: { clanId_userId: { clanId, userId } } })
  if (!existing) {
  ...
  if (existing.status === 'REJECTED' || existing.status === 'LEFT') {
    await prisma.clanMembership.update({
      ...
      data: { status: 'PENDING', role: 'MEMBER', approvedAt: null, approvedById: null },
    })
  }
}

const applyDiscordProfile = async (userId: string, profile: DiscordUserProfile) => {
...
const createUserFromDiscord = async (profile: DiscordUserProfile, clanId?: string) => {
  ...
  await prisma.auditLog.create({
    data: {
      actorUserId: user.id,
      action: clanId ? 'auth.discord_register_pending' : 'auth.discord_register',
      entityType: 'User',
      entityId: user.id,
...
authRouter.get(
  ...
  asyncHandler(async (_req, res) => {
    ...
    res.json({ setupRequired: adminCount === 0, setupTokenRequired: Boolean(env.firstAdminSetupToken) })
  })
)
...
authRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (env.firstAdminSetupToken && input.setupToken !== env.firstAdminSetupToken) {
    ...
    await prisma.auditLog.create({
      data: {
        actorUserId: user.id,
        action: existingUser ? 'auth.setup_admin_promote_existing' : 'auth.setup_admin',
        entityType: 'User',
        entityId: user.id,
      },
    })
...
authRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const redirect = normalizeRelativeRedirect(queryString(req.query.redirect), '/account')
    const authorizeUrl = await createDiscordAuthorizeUrlForRequest(mode as DiscordAuthMode, redirect, queryString(req.query.clanId))
...
authRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const authorizeUrl = await createDiscordAuthorizeUrlForRequest('link', normalizeRelativeRedirect(input.redirect, '/account'))
...
authRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      if (state.mode === 'link') {
        if (!req.auth?.user) {
          res.redirect(toClientRedirect(redirect, { discord: 'login_required' }))
          return
        }

        const existingByDiscord = await prisma.user.findUnique({ where: { discordUserId: profile.id } })
        if (existingByDiscord && existingByDiscord.id !== req.auth.user.id) {
          res.redirect(toClientRedirect(redirect, { discord: 'already_linked' }))
          return
        }
      ...
      let user = await prisma.user.findUnique({ where: { discordUserId: profile.id } })

      if (!user && email) {
        const existingByEmail = await prisma.user.findUnique({ where: { email } })
        if (existingByEmail?.discordUserId && existingByEmail.discordUserId !== profile.id) {
          res.redirect(toClientRedirect(redirect, { discord: 'email_conflict' }))
          return
        }
        if (existingByEmail) {
      ...
      const session = await createSession(user.id)
      setSessionCookie(res, session.token, session.expiresAt)
      res.redirect(toClientRedirect(redirect, { discord: state.mode === 'register' ? 'registered' : 'logged_in' }))
    } catch (error) {
