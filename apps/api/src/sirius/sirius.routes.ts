...
import type { BlueprintStatus, BlueprintSlotGroup, Prisma, SiriusAppearanceStatus, SiriusEnemyType, SiriusPlanetAppearance } from '../generated/prisma/client.js'
...
import { backfillSpawnWindowsForClan, deriveSpawnWindowStatus, isValidNextSpawnAt, nextSpawnAtForRing, normalizeInvalidSpawnSchedulesForClan, syncSpawnWindowForAppearance } from './spawn-windows.js'
...
const ensureCanEditAppearance = async (req: Parameters<typeof hasClanRole>[0], appearanceId: string) => {
  ...
  if (!hasClanRole(req, appearance.clanId, 'LIEUTENANT')) {
    throw new HttpError(403, 'Lieutenant role required for this clan.')
  }
  return appearance
}

const ensureCanEditSpawnWindow = async (req: Parameters<typeof hasClanRole>[0], spawnWindowId: string) => {
  ...
  if (!hasClanRole(req, spawnWindow.clanId, 'LIEUTENANT')) {
    throw new HttpError(403, 'Lieutenant role required for this clan.')
  }
  return spawnWindow
}

export const buildSlotCounts = async (clanId: string, blueprintIds: string[], includeStatusUsers = true, viewerUserId?: string) => {
  ...
  const statusUserIds = Array.from(new Set(viewerUserId ? [...userIds, viewerUserId] : userIds))
  ...
  for (const blueprintId of trackableIds) {
    const viewerStatus = viewerUserId ? statusByUserAndBlueprint.get(`${viewerUserId}:${blueprintId}`) : undefined
    if (viewerStatus) {
...
type JourneyWarning = 'UNLINKED_PLANET' | 'ARRIVAL_OVERDUE' | 'DEPARTURE_OVERDUE' | 'PLANET_EXPIRED' | 'ARRIVES_AFTER_EXPIRY'
...
const ensureCanEditJourneyStop = async (req: Parameters<typeof hasClanRole>[0], stopId: string) => {
  ...
  if (!hasClanRole(req, stop.clanId, 'LIEUTENANT')) {
    throw new HttpError(403, 'Lieutenant role required for this clan.')
  }
  return stop
}
...
const listSerializedJourneyStops = async (clanId: string) => {
  ...
  const { counts: countsByBlueprintId } = await buildSlotCounts(clanId, blueprintIds)
    ...
    res.json({
      ...
      appearances: appearances.map(appearance => ({
        ...
        slots: appearance.slots.map(slot => ({
          ...
          viewerStatus: slot.blueprintId ? (viewerStatuses.get(slot.blueprintId) ?? null) : null,
        })),
      })),
    })
  }),
...
siriusRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    for (const event of events) {
      ...
      const eventStatus = event.appearance?.status ?? (event.expiresAt ? statusForAppearance(event.observedAt ?? event.dropAt, event.expiresAt) : 'EXPIRED')
      row.totalDrops += 1
...
siriusRouter.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const blueprintIds = stop.appearance?.slots.map(slot => slot.blueprintId).filter((id): id is string => Boolean(id)) ?? []
    const { counts: countsByBlueprintId } = await buildSlotCounts(stop.clanId, blueprintIds)
...
siriusRouter.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const nextSpawnAt = nextRing === 5 ? (input.expiresAt || input.ring ? nextSpawnAtForRing(nextRing, nextExpiresAt) : existing.nextSpawnAt) : null
...
siriusRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    scheduleWantedBlueprintAlertsForClan(appearance.clanId, {
      appearanceIds: [appearance.id],
      blueprintIds: [slot.blueprintId].filter((id): id is string => Boolean(id)),
    })
...
siriusRouter.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    scheduleWantedBlueprintAlertsForClan(appearance.clanId, {
      appearanceIds: [appearance.id],
      blueprintIds: slots.map(slot => slot.blueprintId).filter((id): id is string => Boolean(id)),
    })
    res.json({ slots })
  }),
)
...
siriusRouter.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!hasClanRole(req, current.appearance.clanId, 'LIEUTENANT')) {
      throw new HttpError(403, 'Lieutenant role required for this clan.')
    }
