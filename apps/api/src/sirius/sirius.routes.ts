...
import {
  slotGroupsForRing,
  techTierForRing,
  validateSiriusSlotShape,
} from "./sirius.rules.js";
import {
  blueprintSummaryInclude,
  serializeBlueprintSummary,
} from "../blueprints/blueprint.dto.js";
import {
  backfillDropEventsForClan,
  syncDropEventsForAppearance,
} from "./drop-events.js";
import {
...
import { scheduleWantedBlueprintAlertsForClan } from "../notifications/blueprint-alerts.service.js";
...
const visibleAppearanceStatuses: SiriusAppearanceStatus[] = [
  "ACTIVE",
  "UPCOMING",
];

const statusForAppearance = (
  observedAt: Date,
  expiresAt: Date,
): SiriusAppearanceStatus => {
  const now = new Date();
...
const ensureValidSiriusTimeline = (input: {
  observedAt: Date;
  expiresAt: Date;
  ...
  if (input.expiresAt.getTime() <= input.observedAt.getTime()) {
    throw new HttpError(
      400,
      "Sirius expiry time must be after the observed/start time.",
    );
  }
  if (!isValidNextSpawnAt(input.expiresAt, input.nextSpawnAt)) {
    throw new HttpError(
      400,
      "Sirius next spawn time must be after the planet expiry time.",
    );
  }
};
...
const ensureSlotAllowedForAppearance = async (
  ...
  if (!rule) {
    throw new HttpError(
      400,
      "This blueprint cannot drop in the selected Sirius ring and slot.",
    );
  }
};

const resolveAppearancePlanet = async (input: {
  planetId?: string;
  planetName?: string;
  ring: number;
}) => {
...
const ensureCanEditAppearance = async (
...
export const buildSlotCounts = async (
  ...
  for (const blueprintId of trackableIds) {
    ...
    for (const user of users) {
      const status = statusByUserAndBlueprint.get(
        `${user.userId}:${blueprintId}`,
      );
      if (status === "OWNED") {
        item.owned += 1;
...
const getTrackingMemberCounts = async (clanId: string) => {
  const [counted, excluded] = await prisma.$transaction([
    ...
    prisma.clanMembership.count({
      where: { clanId, status: "ACTIVE", trackingExcluded: true },
    }),
  ]);
...
type JourneyStopWithRelations = Prisma.ClanJourneyStopGetPayload<{
  include: typeof journeyStopInclude;
}>;
type JourneyWarning =
  | "UNLINKED_PLANET"
  | "ARRIVAL_OVERDUE"
...
const ensureValidJourneyTimes = (input: {
  arriveAt?: Date | null;
  departAt?: Date | null;
}) => {
  if (
    input.arriveAt &&
    input.departAt &&
    input.departAt.getTime() <= input.arriveAt.getTime()
  ) {
    throw new HttpError(
      400,
      "Journey departure time must be after the arrival time.",
    );
  }
};
...
const resolveJourneyTarget = async (input: {
  clanId: string;
  ...
  if (input.planetId) {
    const planet = await prisma.siriusPlanet.findUnique({
      where: { id: input.planetId },
    });
    if (!planet) {
...
const ensureCanEditJourneyStop = async (
  req: Parameters<typeof hasClanRole>[0],
  stopId: string,
) => {
...
const normalizeCurrentJourneyStop = async (
  clanId: string,
...
const journeyWarningsForStop = (
  stop: JourneyStopWithRelations,
  now = new Date(),
): JourneyWarning[] => {
  ...
  if (
    stop.appearance?.status === "EXPIRED" &&
    (stop.status === "PLANNED" || stop.status === "CURRENT")
  ) {
    warnings.add("PLANET_EXPIRED");
  }
  if (
    stop.arriveAt &&
    stop.appearance?.expiresAt &&
    stop.arriveAt > stop.appearance.expiresAt
  ) {
...
const serializeJourneyStop = (
  stop: JourneyStopWithRelations,
  countsByBlueprintId: Map<string, SlotCountSummary>,
) => {
...
const listSerializedJourneyStops = async (clanId: string) => {
  ...
  const blueprintIds = stops.flatMap(
    (stop) =>
      stop.appearance?.slots
        .map((slot) => slot.blueprintId)
        .filter((id): id is string => Boolean(id)) ?? [],
...
siriusRouter.get(
  ...
  asyncHandler(async (_req, res) => {
    const planets = await prisma.siriusPlanet.findMany({
...
siriusRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const slotGroup = stringQuery(req.query.slotGroup) as
      BlueprintSlotGroup | undefined;
  ...
  asyncHandler(async (req, res) => {
    ...
    const spawnWindows = await prisma.siriusSpawnWindow.findMany({
      where: {
        ...
        OR: [
          { status: "PENDING" },
          { expectedAt: { gte: recentResolvedSince } },
        ],
      },
      include: {
        sourceAppearance: {
    ...
    const rows = spawnWindows
      ...
      .sort((left, right) => {
        const statusDiff =
          statusOrder[left.derivedStatus] - statusOrder[right.derivedStatus];
...
siriusRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    res
      .status(201)
      .json({ stop: serializeJourneyStop(stop, countsByBlueprintId) });
  }),
);
...
siriusRouter.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (stops.length !== input.stopIds.length) {
      throw new HttpError(
        404,
        "One or more journey stops were not found for this clan.",
      );
    }
...
siriusRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const ensureHistoryRow = (
      blueprint: (typeof rules)[number]["blueprint"],
    ): SiriusHistoryRow => {
    ...
    for (const event of events) {
      ...
      const eventStatus =
        event.appearance?.status ??
        (event.expiresAt
          ? statusForAppearance(
              event.observedAt ?? event.dropAt,
              event.expiresAt,
            )
          : "EXPIRED");
      row.totalDrops += 1;
      ...
      if (!row.lastDropAt || dropAt.getTime() > row.lastDropAt.getTime()) {
        ...
        row.lastPartsRequired =
          event.partsRequired ?? event.blueprint.partsRequired;
    ...
    const history = Array.from(historyRows.values()).sort((a, b) => {
      const timeDiff =
        (b.lastDropAt?.getTime() ?? 0) - (a.lastDropAt?.getTime() ?? 0);
...
siriusRouter.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const hasTargetChange =
      input.appearanceId !== undefined ||
      input.planetId !== undefined ||
      input.planetName !== undefined;
    const target = hasTargetChange
      ...
      : null;
    const arriveAt =
      input.arriveAt !== undefined
        ? parseOptionalDate(input.arriveAt)
        : existing.arriveAt;
    const departAt =
      input.departAt !== undefined
        ? parseOptionalDate(input.departAt)
        : existing.departAt;
    ensureValidJourneyTimes({ arriveAt, departAt });
      ...
      data: {
        ...
        notes:
          input.notes === undefined
            ? undefined
            : (normalizeNullableString(input.notes) ?? null),
      },
      include: journeyStopInclude,
    });
...
siriusRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const spawnWindowToResolve = input.resolvesSpawnWindowId
      ? await prisma.siriusSpawnWindow.findUnique({
          where: { id: input.resolvesSpawnWindowId },
        })
      : null;
    ...
    if (
      spawnWindowToResolve?.clanId !== undefined &&
      spawnWindowToResolve.clanId !== clanId
    ) {
    ...
    await syncSpawnWindowForAppearance(
      prisma,
      appearance.id,
      req.auth!.user.id,
    );
    ...
    await prisma.auditLog.create({
      data: {
        ...
        afterJson: {
          appearance,
          resolvesSpawnWindowId: input.resolvesSpawnWindowId ?? null,
        },
      },
    });

    scheduleClanDiscordStatusPublish(clanId);
    scheduleWantedBlueprintAlertsForClan(clanId, {
      appearanceIds: [appearance.id],
    });
  ...
  asyncHandler(async (req, res) => {
    ...
    const planet =
      input.planetId || input.planetName
        ...
        : null;
    const nextObservedAt = input.observedAt
      ? new Date(input.observedAt)
      : existing.observedAt;
    ...
    const nextSpawnAt =
      nextRing === 5
        ? input.expiresAt || input.ring
          ? nextSpawnAtForRing(nextRing, nextExpiresAt)
          : existing.nextSpawnAt
        : null;
    ensureValidSiriusTimeline({
    ...
    const appearance = await prisma.siriusPlanetAppearance.update({
      ...
      data: {
        ...
        status:
          input.observedAt || input.expiresAt
            ? statusForAppearance(nextObservedAt, nextExpiresAt)
            : undefined,
        notes: input.notes,
      },
      include: { planet: true, slots: true },
    ...
    scheduleWantedBlueprintAlertsForClan(appearance.clanId, {
      appearanceIds: [appearance.id],
    });
    res.json({ appearance });
  }),
...
siriusRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    scheduleWantedBlueprintAlertsForClan(appearance.clanId, {
      appearanceIds: [appearance.id],
      blueprintIds: [slot.blueprintId].filter((id): id is string =>
        Boolean(id),
      ),
    });
...
siriusRouter.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    const slots = await prisma.$transaction(async (tx) => {
      await tx.siriusPlanetBlueprintSlot.deleteMany({
  ...
  asyncHandler(async (req, res) => {
    ...
    if (input.slotGroup || input.blueprintId || input.enemyType !== undefined) {
      await ensureSlotAllowedForAppearance(current.appearance, {
        ...
        enemyType:
          input.enemyType === undefined ? current.enemyType : input.enemyType,
      });
    }
    ...
    if (slot.blueprintId) {
      scheduleWantedBlueprintAlertsForClan(current.appearance.clanId, {
        appearanceIds: [current.appearanceId],
        blueprintIds: [slot.blueprintId],
      });
    }
    res.json({ slot });
  }),
...
siriusRouter.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    await prisma.siriusPlanetBlueprintSlot.delete({
      where: { id: current.id },
    });
    await syncDropEventsForAppearance(prisma, current.appearanceId);
    scheduleClanDiscordStatusPublish(current.appearance.clanId);
